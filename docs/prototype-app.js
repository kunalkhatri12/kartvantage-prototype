(function () {
  ...
  function render() {
    ...
    root.innerHTML = `
      ...
      <div class="kv-prototype-bar" role="status"><strong>Prototype mode</strong><span>No Shopify or customer data is changed</span>
        <label>Role <select data-control="role" aria-label="Prototype role">${roles.map(x => `<option${x === state.role ? " selected" : ""}>${x}</option>`).join("")}</select></label>
        ...
        ${button("Reset demo", "reset-demo", "quiet")}
      </div>
      <div class="kv-app-shell" data-menu-open="false">
        ...
        <main id="kv-main" class="kv-main" tabindex="-1"><div class="kv-topbar"><button type="button" class="kv-mobile-menu" data-action="toggle-menu" aria-controls="kv-sidebar" aria-expanded="false">Menu</button><div class="kv-topbar__context"><strong>${r.ops ? "Internal operations" : "Merchant workspace"}</strong>${esc(state.scenario)} scenario · fictional data</div><div class="kv-topbar__actions">${badge("DEMO", "demo")}</div></div><div class="kv-page">${r.ops ? renderOps(r) : renderMerchant(r)}</div></main>
      </div>`;
  ...
  function overview() {
    ...
    return `${header("Clear carts. Confident checkouts.", "Understand, test, and guide every cart before Shopify makes the final checkout decision.", state.scenario==="New store"?button("Start guided setup","go-onboarding","primary"):button("Create a rule", "new-rule", "primary"))}${scenarioNotice()}
      <section class="kv-grid kv-grid--4" aria-label="Workspace summary">${metric("Published rules", published, "Simulated enforcement")}${metric("Draft rules", visibleRules.filter(x => x.status === "draft").length, "Not yet enforced")}${metric("Needs attention", state.scenario === "Healthy" ? "0" : "1", "Review recommended")}${metric("Storefront", state.scenario==="New store"?"Setup needed":state.storefrontActive ? "Active" : "Setup needed", "Guidance only")}</section>
      <section class="kv-card"><div class="kv-section-heading"><div><p class="kv-eyebrow">Guided setup</p><h2>Four calm steps to confidence</h2></div>${badge("3 of 4", "info")}</div><div class="kv-grid kv-grid--4">${["Create a rule", "Test with sample carts", "Add storefront guidance", "Confirm store health"].map((x, i) => `<article class="kv-step"><span>${i < 3 ? "✓" : "4"}</span><h3>${x}</h3><p>${["Choose a merchant-friendly template.", "See exactly why a cart passes or stops.", "Use a theme app extension, never theme edits.", "Review evidence before launch."][i]}</p>${i === 0 ? button("Open rules", "go-rules", "link") : i === 1 ? button("Open Test Lab", "go-test", "link") : i === 2 ? button("Configure", "go-storefront", "link") : button("View health", "go-health", "link")}</article>`).join("")}</div></section>
      <section class="kv-grid kv-grid--2"><article class="kv-card"><h2>How KartVantage works</h2><ol class="kv-flow"><li><b>Create</b><span>Plain-language cart guardrails</span></li><li><b>Test</b><span>Deterministic sample carts</span></li><li><b>Guide</b><span>Theme-safe shopper messaging</span></li><li><b>Enforce</b><span>Shopify Validation has final say</span></li></ol></article><article class="kv-card"><h2>Recent activity</h2>${renderTimeline(4)}</article></section>`;
  }
  ...
  function handleAction(action, target) {
    ...
    const routes = { "new-rule":"app/rules/new", "go-onboarding":"app/onboarding", "go-rules":"app/rules", "go-test":"app/test-lab", "go-storefront":"app/storefront", "go-health":"app/health", "go-settings":"app/settings", "ops-merchants":"app/ops/merchants", "ops-support":"app/ops/support", "ops-publishing":"app/ops/publishes" };
